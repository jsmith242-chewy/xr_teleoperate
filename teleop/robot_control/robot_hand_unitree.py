...
class Dex3_1_Controller_v2:
    def __init__(self, left_hand_array_in, right_hand_array_in, dual_hand_data_lock = None, dual_hand_state_array_out = None,
                       dual_hand_action_array_out = None, fps = 100.0, Unit_Test = False, simulation_mode = False):
        """V2 version of Dex3_1_Controller, that directly uses the input arrays to send to the gripper controller.

        [note] A *_array type parameter requires using a multiprocessing Array, because it needs to be passed to the internal child process

        left_hand_array_in: [input] Left hand data.

        right_hand_array_in: [input] Right hand data.
        ...
        simulation_mode: Whether to use simulation mode (default is False, which means using real robot)
        """
        logger_mp.info("Initialize Dex3_1_Controller_v2...")
        ...
        try:
            while self.running:
                try:
                    ...
                    # get dual hand state
                    with left_hand_array_in.get_lock():
                        left_q_target  = np.array(left_hand_array_in[:]).copy()
                    with right_hand_array_in.get_lock():
                        right_q_target = np.array(right_hand_array_in[:]).copy()
                    ...
                    logger_mp.error(f"An exception has happened while trying to control the hand via Dex3_1_Controller_v2.control_process() Exception: {e}")
        finally:
            logger_mp.info("Dex3_1_Controller has been closed.")
